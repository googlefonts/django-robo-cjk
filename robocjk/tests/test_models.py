...
class ModelsTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self._character_glyph = CharacterGlyph.objects.create(
            font=self._font1, data=self.read_glif_data("characterGlyph/uni4E_25.glif")
        )
    ...
    def test_character_glyph(self):
        ...
        self.assertEqual(
            list(self._character_glyph.layers.all()), [self._character_glyph_layer]
        )
        self.assertEqual(
            list(self._character_glyph.deep_components.all()), [self._deep_component]
        )
    ...
    def test_atomic_element(self):
        ...
        self.assertEqual(
            list(self._atomic_element.layers.all()), [self._atomic_element_layer]
        )
