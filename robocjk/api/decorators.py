    ...
    def wrapper(request, *args, **kwargs):
        ...
        try:
            ...
            return ApiResponseNotFound(
                "Project object with 'project_uid={}' not found or the user have not the necessary permissions.".format(
                    project_uid
                )
            )
        # success
        ...
        if not font_uid:
            ...
            if project_uid and font_name:
                try:
                    ...
                    pass
        if not font_uid:
            return ApiResponseBadRequest("Invalid or missing parameter '{}font_uid'.")
        try:
            ...
            return ApiResponseNotFound(
                "Font object with 'font_uid={}' not found or the user have not the necessary permissions.".format(
                    font_uid
                )
            )
        ...
        filters = benedict(
            {
                ...
                "locked_by_id": params.get_int(
                    "locked_by", user.id if is_locked_by_current_user else None
                ),
                "updated_by_id": params.get_int(
                    "updated_by", user.id if updated_by_current_user else None
                ),
...
def require_data(view_func):
    ...
    def wrapper(request, *args, **kwargs):
        ...
        if not glif.ok:
            return ApiResponseBadRequest(
                'Invalid parameter "data", data must be a valid .glif xml file - {}.'.format(
                    str(glif.error)
                )
            )
        # success
...
def require_atomic_element(**kwargs):
    ...
    def decorator(view_func, *args, **kwargs):
        ...
        def inner(request, *args, **kwargs):
            ...
            try:
                obj = (
                    obj_cls.objects.select_related(*select_related)
...
def require_character_glyph(**kwargs):
    ...
    prefetch_related_defaults = [
        "layers",
        "deep_components",
        "deep_components__atomic_elements",
        "deep_components__atomic_elements__layers",
    ]
    prefetch_related = kwargs.get("prefetch_related", prefetch_related_defaults) or []
    prefix_params = kwargs.get("prefix_params", False)
    check_locked = kwargs.get("check_locked", False)
    ...
    def decorator(view_func, *args, **kwargs):
        ...
        def inner(request, *args, **kwargs):
            ...
            if not filters:
                return ApiResponseBadRequest(
                    "Missing parameter '{}id' or '{}name' or '{}unicode_hex'.".format(
