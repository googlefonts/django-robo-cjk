...
class Command(BaseCommand):
    ...
    help = 'Update all glifs updated_at (inherited from their layers).'

    def __init__(self, *args, **kwargs):
        super(Command, self).__init__(*args, **kwargs)

    def handle(self, *args, **options):
        cg_layers_qs = CharacterGlyphLayer.objects.select_related("glif").all()
        cg_layers_count = cg_layers_qs.count()
        ae_layers_qs = AtomicElementLayer.objects.select_related("glif").all()
            ...
            for glifs_page in glifs_paginator:
                ...
                for glif_obj in glifs_list:
                    if glif_obj.updated_at > glif_obj.glif.updated_at:
                        glif_obj.update_glif_updated_at()
                    glifs_counter += 1
