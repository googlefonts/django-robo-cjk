...
class Command(BaseCommand):
    help = "Verify .rcjk projects by comparing glifs in the database with glif files on file-system."
    ...
    def handle(self, *args, **options):
        ...
        logger.info(f"Start verify - pid: {process_id} - thread: {thread_name}")
        ...
        process_id = os.getpid()
        thread_name = threading.current_thread().name
        logger.info(f"Complete verify - pid: {process_id} - thread: {thread_name}")
